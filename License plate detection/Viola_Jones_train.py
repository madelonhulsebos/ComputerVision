...
def midchar_features(int_im, h, w) :
    ...
    featurevalues = np.zeros((int(np.ceil(w/6))))
    count1 = 1

    # First feature frame
    featurevalues[0] = int(int_im[int(2*np.ceil(h/3)), 6]) - int(int_im[int(np.floor(h/3)), 6]) \
                         - int(int_im[int(h), 6])
    ...
    for x in range(6, w - 6, 6) :
        ...
        black_box1 = int(int_im[int(np.floor(h/3)), x+6]) -  int(int_im[int(np.floor(h/3)), x-1]) # upper black box
        white_box = int(int_im[int(2*np.ceil(h/3)), x+6]) - int(int_im[int(2*np.ceil(h/3)), x-1]) \
                    -  int(int_im[int(np.floor(h/3)), x+6]) + int(int_im[int(np.floor(h/3)), x-1])
        black_box2 = int(int_im[int(h), x+6]) - int(int_im[int(h), x-1]) - int(int_im[int(2*np.ceil(h/3)), x+6]) \
...
feature_height = 80
feature_width = 160
num_features = int(np.ceil((feature_width / 6)) + 1)
...
joblib.dump(bdt_nopca, r'License plate detection/classifiers/vj_bdt_nopca')
joblib.dump(bdt_pca, r'License plate detection/classifiers/vj_bdt_pca')
...
for filename in test_images[0:1]:
