...
for filename in positive_images[20:40] :
    ...
    img_gauss_filtered = cv2.GaussianBlur(img_gray, (5, 5), 0)
    ...
    for contour in contours:
        ...
        intX, intY, intWidth, intHeight = cv2.boundingRect(contour)
        if intWidth < LP_WIDTH/3 and intHeight > 1*LP_HEIGHT/5 : #LP_WIDTH/20 and intWidth < 4*LP_WIDTH/7 and intHeight > 2*LP_HEIGHT/10 and intHeight < 9*LP_HEIGHT/10:
